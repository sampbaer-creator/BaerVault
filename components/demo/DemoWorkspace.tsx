...
const initialState: DemoState = {
  ...
  income: augustBudget.incomeEntries.map((entry)=>({id:entry.id,description:entry.source,amount:entry.amount})),
...
export function DemoWorkspace({ view }: { view:"budget"|"cash-flow"|"investments"|"household"|"settings" }) {
  ...
  const spending=useMemo(()=>state.categories.reduce((sum,c)=>sum+c.entries.reduce((s,e)=>s+e.amount,0),0),[state]); const income=state.income.reduce((sum,e)=>sum+e.amount,0);
  ...
  function submit(event:FormEvent<HTMLFormElement>,kind:string){event.preventDefault();const form=new FormData(event.currentTarget);const name=String(form.get("name")??"").trim();const amount=Number(form.get("amount"));if(!name||!Number.isFinite(amount)||amount<=0)return;
    ...
    if(kind==="income")setState(s=>({...s,income:[{id:createId(),description:name,amount},...s.income]}));
  ...
  if(view==="cash-flow")return <Page title="Cash flow" intro="Income and spending are derived from the same demo budget records."><Notice text={notice}/><Stats items={[["Income",money.format(income)],["Spending",money.format(spending)],["Net cash flow",money.format(income-spending)],["Savings rate",`${income?((income-spending)/income*100).toFixed(1):0}%`]]}/><DemoForm onSubmit={(e)=>submit(e,"income")} action="Add income" nameLabel="Income source" amountLabel="Amount"/><Card title="Income entries">{state.income.map(e=><Row key={e.id} label={e.description} value={money.format(e.amount)}/>)}</Card></Page>;
