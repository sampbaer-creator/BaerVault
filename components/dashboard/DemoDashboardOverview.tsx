  ...
  return (
    ...
    >
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={netWorthHistory[timeframe]} margin={{ top: 12, right: 4, left: 0, bottom: 0 }}>
          <defs>
          ...
          <CartesianGrid vertical={false} stroke="rgba(28, 39, 52, 0.08)" strokeDasharray="3 5" />
          <XAxis
            dataKey="label"
            axisLine={false}
            tickLine={false}
          ...
          <Tooltip content={<ChartTooltip />} cursor={{ stroke: "rgba(31, 77, 59, 0.22)" }} />
          <Area
            type="monotone"
            dataKey="value"
            ...
            activeDot={{ r: 4, fill: "#315f50", stroke: "#ffffff", strokeWidth: 2 }}
          />
        </AreaChart>
      </ResponsiveContainer>
    </div>
...
export function DemoDashboardOverview() {
  ...
  return (
    <div className={styles.dashboard}>
      ...
      <section className={styles.netWorthSurface} aria-labelledby="net-worth-title">
        <div className={styles.netWorthTopline}>
          <div className={styles.netWorthCopy}>
            ...
            <div className={styles.netWorthChange}>
              <span><IconArrowUpRight size={16} stroke={2} aria-hidden="true" />+$1,284.42</span>
              <span>+2.4% this month</span>
            </div>
          </div>
          <div className={styles.timeframes} aria-label="Net worth chart timeframe">
            {(Object.keys(netWorthHistory) as Timeframe[]).map((item) => (
              <button
                type="button"
                className={timeframe === item ? styles.timeframeActive : undefined}
      ...
      <div className={styles.detailGrid}>
        <section className={`${styles.panel} ${styles.budgetPanel}`} aria-labelledby="budget-title">
          <SectionHeading id="budget-title" title="August budget" href="/demo/budget" action="Open budget" />
          <div className={styles.budgetLead}>
            <div>
              <span>Spent</span>
              <strong>$3,210.18</strong>
            </div>
            <p><strong>$1,289.82</strong> remaining of $4,500</p>
          </div>
          <div
          ...
          <div className={styles.budgetCategories}>
            {budgetCategories.map((category) => (
              <div className={styles.budgetCategory} key={category.label}>
                <div>
                  <span className={`${styles.categoryDot} ${styles[category.color]}`} />
                  <span>{category.label}</span>
                </div>
                <p><strong>${category.spent.toLocaleString()}</strong> / ${category.budget.toLocaleString()}</p>
              </div>
            ))}
          </div>
        </section>

        <section className={`${styles.panel} ${styles.investmentPanel}`} aria-labelledby="investment-title">
          <SectionHeading id="investment-title" title="Investments" href="/demo/investments" action="View portfolio" />
            ...
            {investments.map((investment) => (
              <div className={styles.holding} key={investment.symbol}>
                ...
                <div className={styles.holdingValue}>
                  ...
                  <small className={investment.change.startsWith("+") ? styles.positive : styles.negative}>
                    {investment.change}
                  </small>
                </div>
              </div>
        ...
        <section className={`${styles.panel} ${styles.transactionsPanel}`} aria-labelledby="transactions-title">
          <SectionHeading id="transactions-title" title="Recent activity" href="/demo/cash-flow" action="View cash flow" />
          <div className={styles.transactions}>
            {recentTransactions.map((transaction, index) => {
              ...
              return (
                <div className={styles.transaction} key={`${transaction.merchant}-${transaction.date}`}>
                  ...
                  <span className={incoming ? styles.incoming : styles.outgoing}>
                    {incoming ? <IconArrowUpRight size={14} aria-hidden="true" /> : <IconArrowDownRight size={14} aria-hidden="true" />}
                    {transaction.amount}
                  </span>
                </div>
              );
          ...
          <div className={styles.monthlyNote}>
            ...
            <p><strong>On track for August.</strong> Spending is $184 below your typical pace.</p>
          </div>
        </section>
      </div>
    </div>
