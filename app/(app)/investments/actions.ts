...
export async function updateHoldingAction(input:{id:string;symbol:string;name:string}){try{const symbol=input.symbol.trim().toUpperCase(),name=input.name.trim()||symbol;if(!input.id||!/^[A-Z0-9./-]{1,15}$/.test(symbol)||name.length>120)throw new Error("Enter valid holding details.");await updateHolding(input.id,symbol,name);refresh();return{ok:true as const}}catch(error){return{ok:false as const,error:errorMessage(error)}}}
...
export async function updatePurchaseLotAction(input:{id:string;shares:number;price:number;date:string}){try{if(!input.id||input.shares<=0||input.price<0||!/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(input.date))throw new Error("Enter valid lot details.");await updatePurchaseLot(input.id,input.shares,input.price,input.date);refresh();return{ok:true as const}}catch(error){return{ok:false as const,error:errorMessage(error)}}}
