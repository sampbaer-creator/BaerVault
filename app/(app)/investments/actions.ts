...
const refresh=()=>{refreshFinanceViews(); refreshFinanceViews()};
...
export async function addHoldingAction(input: { accountId: string; symbol: string; name: string; shares: number; price: number; date: string }) {
  try {
    ...
    if (!input.accountId || !/^[A-Z0-9./-]{1,15}$/.test(symbol) || name.length > 120 || !Number.isFinite(input.shares) || input.shares <= 0 || !Number.isFinite(input.price) || input.price < 0 || !isValidDate(input.date)) throw new DataAccessError("Enter a valid holding and purchase lot.");
...
export async function addPurchaseLotAction(input: { holdingId: string; shares: number; price: number; date: string }) {
  try {
    if (!input.holdingId || !Number.isFinite(input.shares) || input.shares <= 0 || !Number.isFinite(input.price) || input.price < 0 || !isValidDate(input.date)) throw new DataAccessError("Enter valid lot details.");
