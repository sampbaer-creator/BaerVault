...
export async function deleteGoalAction(id: string) { try { if (!id) throw new DataAccessError("That goal no longer exists."); await deleteSavingsGoal(id); refresh(); return { ok: true as const }; } catch (error) { return { ok: false as const, error: errorMessage(error) }; } }
