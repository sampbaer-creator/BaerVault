...
export async function updateInvestmentAccount(id:string,name:string,accountType:string,ownership:string){const household=await getCurrentHousehold();const result=await createServerSupabaseClient().from("investment_accounts").update({name,account_type:accountType,ownership}).eq("id",id).eq("household_id",household.id).select("id").single();if(result.error)throwDataError(result.error,"Could not update the investment account.");}
export async function deleteInvestmentAccount(id:string){const household=await getCurrentHousehold();const result=await createServerSupabaseClient().from("investment_accounts").delete().eq("id",id).eq("household_id",household.id);if(result.error)throwDataError(result.error,"Could not delete the investment account.");}
export async function updateHolding(id:string,symbol:string,name:string){const household=await getCurrentHousehold();const result=await createServerSupabaseClient().from("holdings").update({symbol,investment_name:name}).eq("id",id).eq("household_id",household.id).select("id").single();if(result.error)throwDataError(result.error,"Could not update the holding.");}
export async function deleteHolding(id:string){const household=await getCurrentHousehold();const result=await createServerSupabaseClient().from("holdings").delete().eq("id",id).eq("household_id",household.id);if(result.error)throwDataError(result.error,"Could not delete the holding.");}
export async function updatePurchaseLot(id:string,shares:number,price:number,date:string){const household=await getCurrentHousehold();const result=await createServerSupabaseClient().from("purchase_lots").update({shares,price_per_share:price,purchase_date:date}).eq("id",id).eq("household_id",household.id).select("id").single();if(result.error)throwDataError(result.error,"Could not update the purchase lot.");}
export async function deletePurchaseLot(id:string){const household=await getCurrentHousehold();const result=await createServerSupabaseClient().from("purchase_lots").delete().eq("id",id).eq("household_id",household.id);if(result.error)throwDataError(result.error,"Could not delete the purchase lot.");}
